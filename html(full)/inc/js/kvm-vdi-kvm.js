...
$(document).ready(function(){
    ...
    $('#main_table').on("click", ".revert", function() { //since table items are dynamically generated, we will not get ordinary .click() event
        var vm = $(this).data('id');
        $.post({
    ...
    $('#main_table').on("click", ".MassSnapshotButton", function(e) { //since table items are dynamically generated, we will not get ordinary .click() event
        ...
        var action = $(this).data('action');
        $.post({
            url: 'inc/infrastructure/KVM/VMSnapshot1.php',
                data: {
                    vm: sourcevm,
                    action: action,
                },
