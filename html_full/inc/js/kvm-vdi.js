...
$(document).ready( function() { 
    ...
    $('.DeleteSnapshotButton').click(function() {
        $("#SubmitButton").removeClass('hide');
        var id = $(this).data('id');
        $('#snapshots-'+id).prop('checked', true);
        $(".snapshot_name-"+id).addClass('users-deleted');
    })
    ...
    $('.showstatus1').click(function() {
        var id = $(this).data('id');
        $("#time-"+id).removeClass('hide');
    })


    $('.RevertSnapshotButton').click(function() {
        var id = $(this).data('id');
        new Date($.now());
        $('#snapshots-'+id).prop('checked', true);
        if (confirm('Are you sure?')){
            var dt = new Date();
            $.post({
                url : 'inc/infrastructure/KVM/revert.php',
                data: {
                    snapshotid : id,
                },
                success:function (data) {
    ...
    $('#SubmitButton').click(function() {
        ...
        if (confirm('Are you sure?')){
            ...
            if ($(this).val() != 'on')
                to_delete.push($(this).val());
            $("#row-snapshot_name-"+$(this).val()).remove();
            });
            $.post({
                url : 'inc/infrastructure/deleteSnapshot.php',
                data: {
                    type : 'delete',
                    credid : to_delete,
                    credential_type : $('#credential_type').val(),
    ...
    $('#snapshotButton').click(function() {
        $.post({
            url : 'inc/infrastructure/create_Snapshot1.php',
            data: {
