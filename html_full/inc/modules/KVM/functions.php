...
function draw_html5_buttons1(){
    require_once ('HTML5Buttons1.php');
    HTML5Buttons();
}
//############################################################################################
...
function draw_dashboard_table(){
    ...
    while ($x<sizeof($sql_reply)){
        ...
        // Lay tat ca cac VM source va VM initial trong moi Hypervisor
        ...
        // Gan $hypervisor_name = IP hoac name cua hypervisor
        ...
        echo '</h1>
            ...
            if (isset($userConfig['table_section-'.$vms_query[$y]['id']])){
                ...
                echo '</td> 
                    ...
                    <td class="col-md-2"><a data-toggle="modal" href="snapshot_info.php?vm=' . $vms_query[$y]['id'] . '&hypervisor=' . $vms_query[$x]['id']  . 'credential_type=snapshot" data-target="#modalWm">SHOW</a> </td>

                    <td class="col-md-2"><a data-toggle="modal" href="create_snapshot.php?vm=' . $vms_query[$y]['id'] . '&hypervisor=' . $vms_query[$x]['id'] . '" data-target="#modalWm">ADD</a> </td>
                ...
                if (empty($vms_query[$y]['os_type']))
                    ...
                    echo '<td class="col-md-3">' . ucfirst($vms_query[$y]['os_type']) . ' &#47; ' . '<i id="VMStatusText-' . $vms_query[$y]['id'] . '">' . $vms_status_display . '</i><h5><small>' . $used_by . '</small></h5>';
                        ...
                        echo '<tr class="table-stripe-ani vdi-font child-' . $vms_query[$y]['id'] . ' collapse ' . $vdi_table_section_collapse . '"> 
                            ...
                            <td class="col-md-2"><a data-toggle="modal" href="snapshot_info.php?vm=' . $VDI_query[$q]['id'] . '&hypervisor=' . $sql_reply[$x]['id']  . 'credential_type=snapshot" data-target="#modalWm">SHOW</a> </td>

                            <td class="col-md-2"><a data-toggle="modal" href="create_snapshot.php?vm=' . $VDI_query[$q]['id'] . '&hypervisor=' . $sql_reply[$x]['id'] . '" data-target="#modalWm">ADD</a> </td>
